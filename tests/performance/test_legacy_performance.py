#!/usr/bin/env python3
"""
Legacy Performance Tests for Golden Path Demo.

Originally ``tests/performance_tests.py``. Moved here under ADR-0015's
re-tier migration. Marked ``performance`` and ``legacy``; opt in with
``pytest -m 'performance and legacy'``.
...
psutil = pytest.importorskip("psutil", reason="legacy perf suite needs psutil")
pytest.importorskip("requests", reason="legacy perf suite needs requests")
pytestmark = [pytest.mark.performance, pytest.mark.legacy]
