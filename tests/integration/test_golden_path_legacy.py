#!/usr/bin/env python3
"""
Legacy Golden Path Demo Test Suite (pre-tier-system).

Originally ``tests/golden_path_tests.py``. Moved here under ADR-0015's
re-tier migration. Marked ``legacy`` so the default ``pytest`` invocation
skips it; opt in with ``pytest -m legacy``.
...
pytest.importorskip("requests", reason="legacy suite needs requests")
