#!/usr/bin/env python3
"""
Legacy Security Scan for Golden Path Demo.

Originally ``tests/security_scan.py``. Moved here under ADR-0015's re-tier
migration. Marked ``security`` and ``legacy``; the modern, focussed scans
live alongside in ``tests/security/test_no_plaintext_credentials.py`` etc.
"""

import pytest

pytest.importorskip("requests", reason="legacy security suite needs requests")
pytestmark = [pytest.mark.security, pytest.mark.legacy]
