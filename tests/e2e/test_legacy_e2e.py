#!/usr/bin/env python3
"""
Legacy End-to-End Integration Tests for Golden Path Demo.

Originally ``tests/test-integration-e2e.py``. Moved here under ADR-0015's
re-tier migration. Marked ``e2e`` and ``legacy``; only collected when
``RUN_E2E=1`` is exported.
...
if os.getenv("RUN_E2E") != "1":
    pytest.skip(
        "Set RUN_E2E=1 to run the legacy e2e suite (requires live cluster + GitHub).",
